 ...
 */
async function fetchRegistryText(url, { optional = false } = {}) {
 ...
 * Prevents directory traversal and ambiguous inputs.
 * @param {string} name
 */
 ...
 * Prompts interactively if no --global/--local flag is given.
 * @param {string} itemName
 * @param {'skill'} type
 * @returns {Promise<'local'|'global'>}
 */
async function resolveScope(itemName, type) {
 ...
 * @param {string} markerFile  'SKILL.md' or 'COURSE.md'
 * @param {boolean} force
 * @returns {Promise<boolean>}
 */
async function confirmOverwrite(destDir, markerFile, force) {
 ...
 */
async function installSkill(itemName, scope, { force = false, index = null } = {}) {
...
/**
 * Install a course from the registry.
 * Courses are always local (learning/{name}/).
 * @param {string} itemName
 * @param {{ force?: boolean, index?: object }} opts
 */
async function installCourse(itemName, { force = false, index = null } = {}) {
  let registryIndex = index;
  if (!registryIndex) {
    const spinner = ora('Fetching registry…').start();
    try {
...
switch (command) {
  ...
  case 'install': {
    ...
    const index = await fetchRegistryIndex();
    if (isSkill) {
      const scope = await resolveScope(itemName, 'skill');
